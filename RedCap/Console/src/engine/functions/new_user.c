  ...
  if (ret != NULL)
    {
      ...
      transac = build_hdr(transac, NEW_USR, HDR_RQST);
      usr = strdup(ret->buffer);
      printf("ret->buffer:login %s\n", ret->buffer);
      ...
      for (i = 0; i < strlen(ret->buffer); i++)
	{
      ...
      if(i == 64)
	{
	  printf("ret->buffer:access %s\n", ret->buffer);
	  access = bitsfield(ret->buffer);
	  transac = build_field_b(transac, ID_USR_ACCESS, access, 8);
	  send_transac(transac);
	}
      else
	Writen(gl_redcap->bridge->fd, "new_user \"user_login\" \"user_password\" \"user_name\" \"user_access\"\n");	
      free(nusr);
      free(npass);
    }
  else
